...
class QPackageDialog(QDialog, FORM_CLASS):
    ...
    def chercherRepertoire(self):
        filename = QtWidgets.QFileDialog.getExistingDirectory(parent=None, caption=QtWidgets.QApplication.translate(
            "select destination", "Select directory to Fling from..."), directory=QtCore.QDir.currentPath())
        if filename:
    ...
    def copierCouches(self):
                    ...
                    if (layer.name() == row.text()):
                        ...
                        if layer.type() == QgsMapLayer.VectorLayer:
                            ...
                            if (self._repertoire != ""):
                                ...
                                if self._listeprojections.currentText() != QtWidgets.QApplication.translate(
                                ...
                                if os.name == 'nt':
                                    QgsVectorFileWriter.writeAsVectorFormat(layer,
                                                                            ...
                                                                            "utf-8",
                                                                            QgsCoordinateReferenceSystem(projection),
                                                                            "ESRI Shapefile")
                                else:
                                    QgsVectorFileWriter.writeAsVectorFormat(layer,
                                                                            self._repertoire.text() + "/" + layer.name() + ".shp",
                                ...
                                if False == messageerreuraffiche:
                                    QMessageBox.critical(self, QtWidgets.QApplication.translate("QPackage", "QPackage"),
                                                         QtWidgets.QApplication.translate("choosedestination",
                            ...
                            #                      self._repertoire.toPlainText() + "\\" + os.path.basename(
                            #                          layer.publicSource()))
                            #     else:
                            #         shutil.copy2(layer.publicSource(),
                            #                      self._repertoire.toPlainText() + "/" + os.path.basename(
                        ...
                        # if the layer is a raster, the plugin must copy the file
                            ...
                            if self._repertoire != "":
                                if os.name == 'nt':
                                    ...
                                    shutil.copy2(layer.publicSource(),
                                                 self._repertoire.text() + "/" + os.path.basename(layer.publicSource()))
        ...
        if messageerreuraffiche == False:
            ...
            QgsMessageLog.logMessage(QgsProject.instance().absoluteFilePath())
            if not os.path.isfile(QgsProject.instance().fileName()):
                if self._projectname.text() != "":
                    ...
                    elif strproject[-4:] == ".qgz":
                        fichierprojet = self._repertoire.text() + "\\" + strproject
                        print('fichierprojet elif : ' + str(fichierprojet))
                        # Décompresser le fichier .qgz
                        with zipfile.ZipFile(fichierprojet, 'r') as zip_ref:
                            zip_ref.extractall(self._repertoire.text())

                        # Trouver le fichier .qgs dans le répertoire extrait
                        qgs_file_path = None
                        for root, dirs, files in os.walk(self._repertoire.text()):
                            for file in files:
                                if file.endswith('.qgs'):
                                    fichierprojet = os.path.join(root, file)
                                    print('fichierprojet sortie boucle for : ' + str(fichierprojet))
                                    break

                    else:
                        QMessageBox.critical(self, QtWidgets.QApplication.translate("QPackage", "QPackage"),
                                             QtWidgets.QApplication.translate("Choisir fichier projet",
                                                                              "Vous devez choisir un nom pour le fichier projet"),
                                             QMessageBox.Ok);
                        messageerreuraffiche = True
            ...
            strproject = os.path.basename(QgsProject.instance().absoluteFilePath()).split('/')[-1]
            QgsProject.instance().write(fichierprojet)
            if strproject[-4:] == ".qgz":
                fichierprojetqgs = self._repertoire.text() + "/" + strproject
                # Décompresser le fichier .qgz
                with zipfile.ZipFile(fichierprojetqgs, 'r') as zip_ref:
                    zip_ref.extractall(self._repertoire.text())
                ...
                for root, dirs, files in os.walk(self._repertoire.text()):
                    for file in files:
                        if file.endswith('.qgs'):
                            fichierprojetqgs = os.path.join(root, file)
                            # print('fichierprojet sortie boucle for : ' + str(fichierprojetqgs))
                            break
            # print('fichierprojet avant DOMtree : ' + str(fichierprojetqgs))
            # Save the project to the new directory
            # we change the path of the layers
            DOMTree = parse(fichierprojetqgs)
                            ...
                            if (self._repertoire.text() != ""):
                                if layer.type() == QgsMapLayer.VectorLayer:
                                    ...
                                    for coucheprojet in maplayers:
                                        coucheprojetnom = coucheprojet.getElementsByTagName('layername')[0].childNodes[
                                            0].data
                                        if coucheprojetnom == layer.name():
                                            ...
                                            replaceText(coucheprojet.getElementsByTagName('datasource')[0],
                                                        "./" + layer.name() + ".shp")
                                            replaceText(coucheprojet.getElementsByTagName('provider')[0], "ogr")
                                            pr = "<spatialrefsys>"
                                            pr = pr + "<proj4>" + str(
                                                QgsCoordinateReferenceSystem(projection).toProj4()) + "</proj4>"
                                            pr = pr + "<srsid>" + str(
                                                QgsCoordinateReferenceSystem(projection).srsid()) + "</srsid>"
                                            pr = pr + "<srid>" + str(
                                                QgsCoordinateReferenceSystem(projection).postgisSrid()) + "</srid>"
                                            pr = pr + "<epsg>" + str(
                                                QgsCoordinateReferenceSystem(projection).authid()) + "</epsg>"
                                            pr = pr + "<description>" + str(QgsCoordinateReferenceSystem(
                                                projection).description()) + "</description>"
                                            pr = pr + "<projectionacronym>" + str(QgsCoordinateReferenceSystem(
                                                projection).projectionAcronym()) + "</projectionacronym>"
                                            pr = pr + "<ellipsoidacronym>" + str(QgsCoordinateReferenceSystem(
                                                projection).ellipsoidAcronym()) + "</ellipsoidacronym>"
                                            pr = pr + "</spatialrefsys>"
                                            #logging.info( pr)
            ...
            file_handle = codecs.open(fichierprojetqgs, "w", encoding="utf_8")
            ...
            QMessageBox.warning(self, QApplication.translate("QPackage", "QPackage"),
                                QApplication.translate("mustclose",
                                                       "You must close Qgis to take into account changes"));
